  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      onTapUp: (d) {
        ...
        final idx = (d.localPosition.dx / step).round().clamp(
          0,
          ventas.length - 1,
        );
        onTap(idx);
...
class _ChartPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.save();
    canvas.clipRect(
      Rect.fromLTWH(0, 0, size.width * progress, size.height + 4),
    );
    ...
    for (int i = 1; i < ventas.length; i++) {
      fill.lineTo(xOf(i), yOf(ventas[i]));
    }
    fill
      ..lineTo(xOf(ventas.length - 1), size.height)
      ..close();
    canvas.drawPath(
      fill,
      Paint()
        ..shader = LinearGradient(
          ...
          colors: [
            const Color(0xFF0D4F6C).withValues(alpha: 0.14),
            Colors.transparent,
          ],
    ...
    for (int i = 1; i < ventas.length; i++) {
      line.lineTo(xOf(i), yOf(ventas[i]));
    }
    canvas.drawPath(
      line,
      Paint()
    ...
    if (selectedIndex != null) {
      ...
      final cx = xOf(si);
      final cy = yOf(ventas[si]);
      canvas.drawLine(
        Offset(cx, 0),
        Offset(cx, size.height),
        Paint()
          ..color = const Color(0xFF0D4F6C).withValues(alpha: 0.15)
          ..strokeWidth = 1,
      );
